...
async function start(req, env) {
  ...
  const t = await r.text();
  if (!r.ok) throw Error(`Wan request failed (${r.status}): ${t.slice(0, 700)}`);
...
(function(){
  ...
  function findVideo(x){if(!x)return null;if(typeof x==="string"&&x.indexOf("http")===0)return x;if(Array.isArray(x)){for(var i=0;i<x.length;i++){var u=findVideo(x[i]);if(u)return u;}}if(typeof x==="object"){for(var k in x){var v=findVideo(x[k]);if(v)return v;}}return null;}
  function imageSelected(input){var file=input.files&&input.files[0],s=document.getElementById("imageStatus"),p=document.getElementById("preview");p.innerHTML="";if(!file){s.textContent="No image selected.";return;}s.textContent="IMAGE LOADED ✓  "+file.name+"  ("+Math.round(file.size/1024)+" KB, "+(file.type||"unknown")+")";debug("Browser has the image File object. Ready to send it to Cloudflare.");try{var url=URL.createObjectURL(file);var img=document.createElement("img");img.className="preview";img.src=url;img.alt="Selected starting image";p.appendChild(img);}catch(e){debug("Preview error: "+e.message);}}
  async function generateSceneVideo(btn){
    ...
    debug("Image: "+file.name+" | "+file.size+" bytes | "+(file.type||"unknown"));
    ...
    try{
      ...
      var controller=new AbortController(),timer=setTimeout(function(){controller.abort();},90000);var r;
      try{r=await fetch("/api/wan/start?nocache="+Date.now(),{method:"POST",body:fd,cache:"no-store",signal:controller.signal});}finally{clearTimeout(timer);}
      ...
      debug("Hugging Face event ID received: "+j.eventId);
      ...
      while(true){
        ...
        var sep=String.fromCharCode(10)+String.fromCharCode(10),parts=buf.split(sep);buf=parts.pop()||"";
        for(var i=0;i<parts.length;i++){
          ...
          if(em==="complete"){
            ...
            if(u)document.getElementById("result").innerHTML='<video class="video" controls autoplay src="'+u+'"></video><p><a target="_blank" href="'+u+'">Open generated video</a></p>';else document.getElementById("result").textContent=JSON.stringify(d,null,2);return;
          }
        }
      }
  ...
  window.addEventListener("DOMContentLoaded",function(){
    var image=document.getElementById("image"),generate=document.getElementById("generate"),add=document.getElementById("addCharacter");
    image.addEventListener("change",function(){imageSelected(image);});
    generate.addEventListener("click",function(){generateSceneVideo(generate);});
    add.addEventListener("click",addCharacter);
    status("JavaScript loaded ✓. Choose an image.");
    debug("Framework v9 browser handlers attached successfully.");
  });
})();
</script></body></html>`;
export default { async fetch(req, env) { const u=new URL(req.url); try { if(u.pathname==="/api/health") return json({ok:true,version:"v9",tokenConfigured:Boolean(env.HF_TOKEN),hfSpace:env.HF_SPACE||null}); if(u.pathname==="/api/wan/start"&&req.method==="POST") return await start(req,env); if(u.pathname==="/api/wan/stream"&&req.method==="GET") return await stream(u.searchParams.get("eventId"),env); return new Response(HTML,{headers:{"content-type":"text/html;charset=utf-8","cache-control":"no-store, no-cache, must-revalidate","pragma":"no-cache","expires":"0","x-framework-version":"v9"}}); } catch(e) { return json({ok:false,error:e.message},500); } } };