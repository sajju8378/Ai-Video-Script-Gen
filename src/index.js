...
async function start(req, env) {
  ...
  const duration = Number(f.get("duration") || 3.5); const image = await upload(img, env);
  ...
  return json({ ok: true, eventId: x.event_id });
}
...
async function generateSceneVideo(btn){
 ...
 if(!file){status("Please choose a starting image first.");return;}
 btn.disabled=true; document.getElementById("result").innerHTML="";
 try{
  status("Uploading image: "+file.name+"...");
  var fd=new FormData();fd.append("image",file);fd.append("prompt",document.getElementById("prompt").value);fd.append("duration",document.getElementById("duration").value);
  var r=await fetch("/api/wan/start?nocache="+Date.now(),{method:"POST",body:fd,cache:"no-store"});
  ...
  status("Job submitted, waiting for ZeroGPU response...");
  ...
  while(true){
   ...
   for(var i=0;i<parts.length;i++){
    ...
    if(em==="generating"||em==="pending"||em==="start"){status("Wan 2.2 is generating...");continue;}
 ...
 }catch(e){status("Error: "+(e&&e.message?e.message:String(e)));}finally{btn.disabled=false;}
}
</script></body></html>`;
export default { async fetch(req, env) { const u=new URL(req.url); try { if(u.pathname==="/api/health") return json({ok:true,version:"v7",tokenConfigured:Boolean(env.HF_TOKEN),hfSpace:env.HF_SPACE||null}); if(u.pathname==="/api/wan/start"&&req.method==="POST") return await start(req,env); if(u.pathname==="/api/wan/stream"&&req.method==="GET") return await stream(u.searchParams.get("eventId"),env); return new Response(HTML,{headers:{"content-type":"text/html;charset=utf-8","cache-control":"no-store, no-cache, must-revalidate","pragma":"no-cache","expires":"0","x-framework-version":"v7"}}); } catch(e) { return json({ok:false,error:e.message},500); } } };