...
async function upload(file,env){const fd=new FormData();fd.append('files',file,file.name||'input.png');const r=await fetch(hfBase(env)+'/gradio_api/upload',{method:'POST',headers:auth(env),body:fd});const t=await r.text();if(!r.ok)throw Error('Hugging Face upload failed ('+r.status+'): '+t.slice(0,700));let x;try{x=JSON.parse(t)}catch{throw Error('Hugging Face returned invalid upload data: '+t.slice(0,300))}const path=Array.isArray(x)?x[0]:x;if(!path)throw Error('Hugging Face returned no uploaded file path');return{path,meta:{_type:'gradio.FileData'},orig_name:file.name||'input.png'}}
async function startVideo(req,env){try{const fd=await req.formData();const image=fd.get('image');if(!(image instanceof File))return json({error:'Please choose a starting image.'},400);const prompt=String(fd.get('prompt')||'high quality, high resolution, cinematic motion, smooth animation');const duration=Number(fd.get('duration')||3.5);if(!Number.isFinite(duration)||duration<2||duration>12)return json({error:'Invalid video duration.'},400);const input=await upload(image,env);const data=[input,null,prompt,4,NEG,duration,1,1,42,true,5,'UniPCMultistep',3,16,false,true];const r=await fetch(hfBase(env)+'/gradio_api/call/generate_video',{method:'POST',headers:{...auth(env),'content-type':'application/json'},body:JSON.stringify({data})});const t=await r.text();if(!r.ok)throw Error('Wan request failed ('+r.status+'): '+t.slice(0,800));let x;try{x=JSON.parse(t)}catch{throw Error('Wan returned invalid JSON: '+t.slice(0,500))}if(!x?.event_id)throw Error('Wan returned no event id: '+t.slice(0,700));return json({ok:true,eventId:x.event_id})}catch(e){return json({ok:false,error:e?.message||String(e)},502)}}
async function streamVideo(id,env){if(!id)return json({error:'Missing eventId'},400);const r=await fetch(hfBase(env)+'/gradio_api/call/generate_video/'+encodeURIComponent(id),{headers:auth(env)});if(!r.ok)return new Response(await r.text(),{status:r.status,headers:{'content-type':'text/plain;charset=utf-8'}});return new Response(r.body,{headers:{'content-type':'text/event-stream;charset=utf-8','cache-control':'no-cache,no-store','connection':'keep-alive'}})}
...
function updateCounter(){var n=words(scriptEl.value),ok=n>=MIN_WORDS;countEl.textContent=n+' words — '+(ok?'minimum reached ✓':(MIN_WORDS-n)+' more required');countEl.className='status '+(ok?'good':'bad');validationEl.className='status '+(ok?'good':'bad');validationEl.textContent=ok?'Script ready ✓. Develop Scenes is enabled.':'Please enter at least 20 words before developing scenes. Current count: '+n+'.';developBtn.disabled=!ok;if(ok)devStatus.textContent='Script ready ✓. Press Develop Scenes.';else devStatus.textContent='Type at least '+(MIN_WORDS-n)+' more word'+(MIN_WORDS-n===1?'':'s')+'.'}
...
function motion(t){var x=t.toLowerCase(),p=[];if(/fly|flying|run|walk|move|fight|jump|swim|land|drive|ride/.test(x))p.push('natural character movement');if(/look|looks|watch|turn|face/.test(x))p.push('subtle head and eye movement');if(/wind|ocean|wave|waves|rain|cloud|storm|fire|smoke/.test(x))p.push('natural environmental motion');p.push('smooth cinematic camera movement','high quality','high resolution');return p.join(', ')}
...
export default{async fetch(request,env){const url=new URL(request.url);try{if(request.method==='POST'&&url.pathname==='/api/wan/start')return await startVideo(request,env);if(request.method==='GET'&&url.pathname==='/api/wan/stream')return await streamVideo(url.searchParams.get('eventId'),env);if(request.method==='GET'&&url.pathname==='/api/wan/file')return await videoFile(request,env);return new Response(HTML,{headers:{'content-type':'text/html;charset=utf-8','cache-control':'no-store'}})}catch(e){return json({ok:false,error:e?.message||String(e)},500)}}};
